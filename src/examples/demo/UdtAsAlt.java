...
/**
 * This is a demonstration of how to use a UDT as a specialized alternate (ALT)
 * operation. Despite the name, User Defined Terminal (UDT), UDTs can be used as
 * non-terminal operators as well. See the source code for details.
 */
public class UdtAsAlt extends RunTests {
     ...
     */
    public UdtAsAlt(String testName, PrintStream out) {
        super(testName, out);
    }
     ...
     * always possible to do this syntactically, since a domainlabel which
     * begins with ALPHA and a toplabel followed by the optional "." are
     * syntactially identical. The handwritten, custom ALT operator uses the
     * following 3 differences to distinguish between them. 1. toplabel must
     * always begin with ALPHA whereas domainlabel may begin with DIGIT 2.
     * toplabel may not be followed by a period, "." 3. if toplabel is followed
     * by period, the period must be followed by a space
     *
     */
    class UDomainOrTop extends UdtCallback {
        ...
        UDomainOrTop(Parser parser) {
            super(parser);
        }

        @Override
        public int callback(int offset) throws Exception {
            int i = offset;
            ...
            while (i < inlen) {
                ...
                if (end <= 0) {
                    break;
                } // not a hostname, let the parse fail
                len += end;
                i += end;
