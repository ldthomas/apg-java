...
/**
 * This abstract class is the base class for all examples. It provides many
 * functions for standardizing the tests and their output format.
 */
public abstract class RunTests {
    ...
    protected static class RunInput {
        ...
        public RunInput(String title, Parser parser, int startRule) throws Exception {
            if (parser == null) {
                throw new Exception("RunInput: parser may not be null");
            }
            if (startRule < 0) {
                throw new Exception("RunInput: start rule id may not be < 0");
            }
            if (title == null) {
                this.title = "<no title>";
            } else if (title.length() > 70) {
                this.title = title.substring(67) + "...";
            } else {
                this.title = title;
            }
            this.parser = parser;
    ...
    protected void runComparisionOfCfgAndUdt() throws Exception {
        ...
        boolean test = true;
        if (!(testStrings(cfgIn, inputStrings, true))) {
            test = false;
        }
        out.println();
        outputUdtTestStrings();
        if (!(testStrings(udtIn, inputStrings, true))) {
            test = false;
        }
        out.println();
        if (test) {
    ...
    protected boolean testStrings(RunInput in, String[] strings, boolean traceOnError) throws Exception {
        ...
        for (String str : strings) {
            ...
            if (result.success()) {
                out.println("[" + index + "] success");
            } else {
                out.println("[" + index + "] *** parse failed");
            }
    ...
    private class TimeCompare {
        ...
        public void setFraction(long lhs, long rhs) {
            fraction = (rhs == 0) ? 0.0 : (double) lhs / (double) rhs;
        }
    ...
    protected void outputTestName() {
        out.println("TEST NAME: " + testName);
    }

    protected void outputAbstract(String text) {
        out.println(" ABSTRACT: " + text);
    }

    protected void outputCfgGrammar() {
        out.println("CFG GRAMMAR: (" + testName + ")");
    }

    protected void outputUdtGrammar() {
        out.println("UDT GRAMMAR: (" + testName + ")");
    }

    protected void outputInputStrings() {
        out.println("INPUT STRINGS: (" + testName + ")");
    }

    protected void outputCfgTestStrings() {
        out.println("CFG PARSER: TEST ALL STRINGS: (" + testName + ")");
    }

    protected void outputUdtTestStrings() {
        out.println("UDT PARSER: TEST ALL STRINGS: (" + testName + ")");
    }

    protected void outputTimeComparison() {
        out.println("TIME COMPARISON: (" + testName + ", reps=" + reps + ")");
    }

    protected void outputNodeHits() {
        out.println("NODE HIT COMPARISON: (" + testName + ", reps=" + reps + ")");
    }

    protected void outputCfgNodeStats() {
        out.println("CFG: NODE STATISTICS: (" + testName + ",reps=" + reps + ")");
    }

    protected void outputUdtNodeStats() {
        out.println("UDT: NODE STATISTICS: (" + testName + ", reps=" + reps + ")");
    }

    protected void outputAst(boolean xml) {
        if (xml) {
            out.println("AST IN XML FORMAT: (" + testName + ")");
        } else {
            out.println("AST IN APG FORMAT: (" + testName + ")");
        }
    }

    protected void outputTrace(boolean xml) {
        if (xml) {
            out.println("PARSE FAILED: TRACE IN XML FORMAT: (" + testName + ")");
        } else {
            out.println("PARSE FAILED: TRACE IN APG FORMAT: (" + testName + ")");
        }
    }
}
//protected RunInput     cfgIn            = null;
